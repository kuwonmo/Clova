...
base_url = "http://license.korcham.net/kor/license/schedule.jsp?cd=0102&mm=22"
...
def collecting(base_url1):
    ...
    while True:
        ...
        구분2 = table2[2].text
        등급2 = table2[3].text
        sql = """insert into 자격증(이름, 시험등급, 시험방법, 시험과목, 출제형태, 시험시간, 필기, 실기, 필실가격, 입실시간, 시작시간) values ('워드', %s, %s, %s, %s, %s, %s, %s, %s, %s ,%s)"""
        cursor.execute(sql, (종목, 회별, 구분, 등급, 인터넷접수, 필기, 실기, 필실가격, 입실시간, 시작시간))
        cursor.execute(sql, (종목, 종목2, 회별2, 구분2, 등급2,  필기, 실기, 필실가격, 입실시간, 시작시간))
        db.commit()
